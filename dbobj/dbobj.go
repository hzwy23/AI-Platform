...
func scanForStruct(obj reflect.Value, rows *sql.Rows) error {
	...
	for rows.Next() {
		for j := 0; j < size; j++ {
			...
			case reflect.Int64, reflect.Int32, reflect.Int16, reflect.Int, reflect.Int8, reflect.Uint64, reflect.Uint32, reflect.Uint, reflect.Uint8:
		...
		for index, vals := range argList {
			...
			case *sql.NullInt64:
				kd := obj.Field(index).Kind()
				if kd == reflect.Uint8 || kd == reflect.Uint32 || kd == reflect.Uint64 {
					obj.Field(index).SetUint(uint64(vals.(*sql.NullInt64).Int64))
				} else {
...
func scanForSlice(obj reflect.Value, rows *sql.Rows) error {
	...
	for rows.Next() {
		...
		for index, vals := range argList {
			...
			case *sql.NullInt64:
				kd := obj.Index(i).Field(index).Kind()
				if kd == reflect.Uint8 || kd == reflect.Uint32 || kd == reflect.Uint64 {
					obj.Index(i).Field(index).SetUint(uint64(vals.(*sql.NullInt64).Int64))
				} else {

...
// 接收广播消息
func broadcast(context *Context) (int, string) {
...
func updateStatus()  {
	for {
		...
		for _, item := range rst {
			lock.RLock()
			if _, ok := deviceScan[item.SerialNumber]; !ok {
				// 设备下限
				logger.Warn("设备",item.SerialNumber,"无法被扫描到")
				dbobj.Exec("update device_manage_info set device_status = 4 where serial_number = ? and delete_status = 0", item.SerialNumber)
			}
			lock.RUnlock()
		}

...
func Broadcast(ip string, port string) {
	...
	logger.Info("start listen broadcast, broadcast ip is:", ip, ", port is:", port)
	fmt.Println("start listen broadcast, broadcast ip is:", ip, ", port is:", port)
	for {
		NewUDPPlatformServer(conn)

...
func (r *deviceManageInfoDaoImpl) FindBySerialNumber(serialNumber string) (entity.DeviceManageInfo, error) {
	...
	err := dbobj.QueryForStruct("select device_id, serial_number, device_name, dhcp_flag, device_ip, device_port, device_status, device_attribute, device_power, device_light_threshold, device_brightness, device_temperature, auto_start_time, auto_end_time, light_mode, mac_address, firmware_version, '', '', mask, gateway, pin, create_by, create_date, update_by, update_data, delete_status, power_total, strobe_count from device_manage_info where delete_status = 0 and serial_number = ?", &rst, serialNumber)
	return rst, err
}
...
func (r *deviceManageInfoDaoImpl) Insert(item entity.DeviceManageInfo, GroupId string) (int64, error) {
	...
	result, err := tx.Exec("insert into device_manage_info(device_id, serial_number, device_name, dhcp_flag, device_ip, device_port, device_status, device_attribute, device_power, device_light_threshold, device_brightness, device_temperature, auto_start_time, auto_end_time, light_mode, mac_address, firmware_version, mask, gateway, pin, create_by, create_date, update_by, update_data, delete_status, power_total, strobe_count) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",

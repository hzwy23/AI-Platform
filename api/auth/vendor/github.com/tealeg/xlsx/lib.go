...
func calculateMaxMinFromWorksheet(worksheet *xlsxWorksheet) (minx, miny, maxx, maxy int, err error) {
	...
	for _, row := range Row {
		for _, cell := range C {
			x, y, err = GetCoordsFromCellIDString(R)
			if err != nil {
				return -1, -1, -1, -1, err
			}
			if x < minx {
...
func makeRowFromRaw(rawrow xlsxRow, sheet *Sheet) *Row {
	...
	for _, rawcell := range C {
		if R != "" {
			x, _, error := GetCoordsFromCellIDString(R)
			if error != nil {
...
func formulaForCell(rawcell xlsxC, sharedFormulas map[int]sharedFormula) string {
	...
	if T == "shared" {
		...
		if err != nil {
			...
			if Ref != "" {
				res = Content
				sharedFormulas[Si] = sharedFormula{x, y, res}
			} else {
				sharedFormula := sharedFormulas[Si]
...
func fillCellData(rawcell xlsxC, reftable *RefTable, sharedFormulas map[int]sharedFormula, cell *Cell) {
	...
	if len(data) > 0 {
		...
		case "s": // Shared String
			...
			cell.Value = ResolveSharedString(ref)
...
func readRowsFromSheet(Worksheet *xlsxWorksheet, file *File, sheet *Sheet) ([]*Row, []*Col, int, int) {
	...
	if len(Ref) > 0 {
		minCol, minRow, maxCol, maxRow, err = getMaxMinFromDimensionRef(Ref)
	} else {
	...
	for rowIndex := 0; rowIndex < len(Row); rowIndex++ {
		...
		if len(Spans) != 0 && strings.Count(Spans, ":") == 1 {
			row = makeRowFromSpan(Spans, sheet)
		} else {
		...
		for _, rawcell := range C {
			...
			x, _, _ := GetCoordsFromCellIDString(R)
			...
			if cellX < len(row.Cells) {
				...
				if styles != nil {
					cell.style = getStyle(S)
					cell.NumFmt = getNumberFormat(S)
				}
				cell.date1904 = Date1904
				// Cell is considered hidden if the row or the column of this cell is hidden
				cell.Hidden = Hidden || (len(cols) > cellX && Hidden)
				insertColIndex++
			}
		}
...
func readSheetViews(xSheetViews xlsxSheetViews) []SheetView {
	if SheetView == nil || len(SheetView) == 0 {
		return nil
	}
	sheetViews := []SheetView{}
	for _, xSheetView := range SheetView {
		sheetView := SheetView{}
		if Pane != nil {
			xlsxPane := Pane
			pane := &Pane{}
...
func readSheetFromFile(sc chan *indexedSheet, index int, rsheet xlsxSheet, fi *File, sheetXMLMap map[string]string) (errRes error) {
	...
	worksheet, error := getWorksheetFromSheet(rsheet, worksheets, sheetXMLMap)
	if error != nil {
		result.Error = error
		sc <- result
		return error
	...
	sheet := new(Sheet)
	File = fi
	Rows, Cols, MaxCol, MaxRow = readRowsFromSheet(worksheet, fi, sheet)
	Hidden = rsheet.State == sheetStateHidden || rsheet.State == sheetStateVeryHidden
	SheetViews = readSheetViews(SheetViews)
...
func readSheetsFromZipFile(f *zip.File, file *File, sheetXMLMap map[string]string) (map[string]*Sheet, []*Sheet, error) {
	...
	Date1904 = workbook.WorkbookPr.Date1904

	for entryNum := range workbook.DefinedNames.DefinedName {
		DefinedNames = append(DefinedNames, &workbook.DefinedNames.DefinedName[entryNum])
	}

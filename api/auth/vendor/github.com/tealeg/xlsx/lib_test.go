...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
  ...
  <pageMargins left="0.7" right="0.7"
	...
	c.Assert(err, IsNil)
	file := new(File)
	referenceTable = MakeSharedStringRefTable(sst)
	sheet := new(Sheet)
	...
	c.Assert(Height, Equals, 123.45)
	c.Assert(isCustom, Equals, true)
	cell1 := Cells[0]
	...
	c.Assert(Hidden, Equals, false)
	c.Assert(len(SheetView), Equals, 1)
	sheetView := SheetView[0]
	c.Assert(Pane, NotNil)
	pane := Pane
	c.Assert(XSplit, Equals, 0.0)
	c.Assert(YSplit, Equals, 1.0)
}
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:mx="http://schemas.microsoft.com/office/mac/excel/2008/main" xmlns:mc="http://schemas.openxmlformats.org/markup-compatibility/2006" xmlns:mv="urn:schemas-microsoft-com:mac:vml" xmlns:x14="http://schemas.microsoft.com/office/spreadsheetml/2009/9/main" xmlns:x14ac="http://schemas.microsoft.com/office/spreadsheetml/2009/9/ac" xmlns:xm="http://schemas.microsoft.com/office/excel/2006/main">
  ...
  <drawing r:id="rId1"/>
	...
	row := rows[0] //
	cell1 := Cells[0]
	c.Assert(HMerge, Equals, 1)
	c.Assert(VMerge, Equals, 0)
}
...
`)
	...
	c.Assert(Width, Equals, 0.0)
	c.Assert(Width, Equals, 0.0)
	c.Assert(Width, Equals, 17.0)
	c.Assert(Width, Equals, 18.0)
}
...
`)
	...
	row := rows[2]
	c.Assert(Sheet, Equals, sheet)
	c.Assert(len(Cells), Equals, 3)

	cell1 := Cells[0]
	c.Assert(Value, Equals, "No")

	cell2 := Cells[1]
	c.Assert(Value, Equals, "")

	cell3 := Cells[2]
	c.Assert(Value, Equals, "Yes")
...
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"
  ...
  <pageMargins left="0.7" right="0.7"
	...
	row := rows[1]
	c.Assert(Formula(), Equals, "2*B1")
	c.Assert(Formula(), Equals, "2*C1")
}
...
func (l *LibSuite) TestRoundTripFileWithNoSheetCols(c *C) {
	...
	c.Assert(err, IsNil)
	Save("testdocs/after_write.xlsx")

...
func (r *RowSuite) TestWriteStruct(c *C) {
	...
	if c0, err = String(); err != nil {
		c.Error(err)
	}
	c1, e1 := Int()
	c2, e2 := Float()
	c3 := Bool()
	if c4, err = String(); err != nil {
		c.Error(err)
	}
	if c5, err = String(); err != nil {
		c.Error(err)
	}
	if c6, err = Float(); err != nil {
		c.Error(err)
	}
...
func (r *RowSuite) TestWriteSlice(c *C) {
	...
	s7_ret := WriteSlice(s7, -1)
	c.Assert(s7_ret, Equals, -1)
	s7_ret = WriteSlice(&s7, -1)
	c.Assert(s7_ret, Equals, -1)
	s7_ret = WriteSlice([]string{s7}, -1)
	c.Assert(s7_ret, Equals, -1)
}

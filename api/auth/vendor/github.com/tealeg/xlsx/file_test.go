...
func (l *FileSuite) TestReadSharedStringsFromZipFile(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(referenceTable, NotNil)
}

// Helper function used to test contents of a given xlsxXf against
// expectations.
func testXf(c *C, result, expected *xlsxXf) {
	c.Assert(ApplyAlignment, Equals, ApplyAlignment)
	c.Assert(ApplyBorder, Equals, ApplyBorder)
	c.Assert(ApplyFont, Equals, ApplyFont)
	c.Assert(ApplyFill, Equals, ApplyFill)
	c.Assert(ApplyProtection, Equals, ApplyProtection)
	c.Assert(BorderId, Equals, BorderId)
	c.Assert(FillId, Equals, FillId)
	c.Assert(FontId, Equals, FontId)
	c.Assert(NumFmtId, Equals, NumFmtId)
}
...
func (l *FileSuite) TestReadStylesFromZipFile(c *C) {
	...
	xlsxFile, err = OpenFile("./testdocs/testfile.xlsx")
	c.Assert(err, IsNil)
	c.Assert(styles, NotNil)

	fontCount = len(Font)
	c.Assert(fontCount, Equals, 4)

	font = Font[0]
	c.Assert(Val, Equals, "11")
	c.Assert(Val, Equals, "Calibri")
	...
	c.Assert(Alignment, NotNil)
	c.Assert(Horizontal, Equals, "general")
	c.Assert(Indent, Equals, 0)
	c.Assert(ShrinkToFit, Equals, false)
	c.Assert(TextRotation, Equals, 0)
	c.Assert(Vertical, Equals, "bottom")
	c.Assert(WrapText, Equals, false)
...
func (l *FileSuite) TestReadWorkbookRelationsFromZipFile(c *C) {
	...
	xlsxFile, err = OpenFile("./testdocs/testfile.xlsx")
	c.Assert(err, IsNil)
	c.Assert(len(Sheets), Equals, 3)
	sheet, ok := Sheet["Tabelle1"]
...
func (l *FileSuite) TestGetStyleFromZipFile(c *C) {
	...
	c.Assert(BgColor, Equals, "FF33CCCC")
	c.Assert(ApplyFill, Equals, false)
...
func (l *FileSuite) TestCreateSheet(c *C) {
	...
	sheet = Sheet["Tabelle1"]
	rowLen := len(Rows)
	c.Assert(rowLen, Equals, 2)
	row = Rows[0]
	c.Assert(len(Cells), Equals, 2)
	cell := Cells[0]
	if val, err := String(); err != nil {
		c.Error(err)
	} else {
...
func (l *FileSuite) TestAddSheet(c *C) {
	...
	f = NewFile()
	sheet, err := AddSheet("MySheet")
	c.Assert(err, IsNil)
	c.Assert(sheet, NotNil)
	c.Assert(len(Sheets), Equals, 1)
	c.Assert(Sheet["MySheet"], Equals, sheet)
}
...
func (l *FileSuite) TestAppendSheet(c *C) {
	...
	f = NewFile()
	s := Sheet{}
	sheet, err := AppendSheet(s, "MySheet")
	c.Assert(err, IsNil)
	c.Assert(sheet, NotNil)
	c.Assert(len(Sheets), Equals, 1)
	c.Assert(Sheet["MySheet"], Equals, sheet)
...
func (l *FileSuite) TestNthSheet(c *C) {
	...
	sheetByIndex := Sheets[0]
	sheetByName := Sheet["MySheet"]
...
func (l *FileSuite) TestMarshalWorkbook(c *C) {
	...
	AddSheet("MyFirstSheet")
	AddSheet("MySecondSheet")
...
func (l *FileSuite) TestMarshalFile(c *C) {
	var f *File
	f = NewFile()
	sheet1, _ := AddSheet("MySheet")
	row1 := AddRow()
	cell1 := AddCell()
	SetString("A cell!")
	sheet2, _ := AddSheet("AnotherSheet")
	row2 := AddRow()
	cell2 := AddCell()
	SetString("A cell!")
...
func (l *FileSuite) TestSaveFile(c *C) {
	...
	c.Assert(err, IsNil)
	c.Assert(xlsxFile, NotNil)
	c.Assert(len(Sheets), Equals, 2)

	sheet1, ok := Sheet["MySheet"]
	c.Assert(ok, Equals, true)
	c.Assert(len(Rows), Equals, 1)
	row1 = Rows[0]
	c.Assert(len(Cells), Equals, 1)
	cell1 = Cells[0]
	c.Assert(Value, Equals, "A cell!")
}

...
func testSetup() {
	...
	protoGroup = NewGroup(protoGroupName, cacheSize, GetterFunc(func(_ Context, key string, dest Sink) error {
		...
		Add(1)
		return SetProto(&testpb.TestMessage{
...
func TestGetDupSuppressString(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		go func() {
			var s string
			if err := Get(dummyCtx, fromChan, StringSink(&s)); err != nil {
...
func TestGetDupSuppressProto(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		go func() {
			...
			if err := Get(dummyCtx, fromChan, ProtoSink(tm)); err != nil {
...
func TestCaching(t *testing.T) {
	...
	fills := countFills(func() {
		for i := 0; i < 10; i++ {
			var s string
			if err := Get(dummyCtx, "TestCaching-key", StringSink(&s)); err != nil {
				t.Fatal(err)
			}
		}
	})
...
func TestCacheEviction(t *testing.T) {
	...
	getTestKey := func() {
		var res string
		for i := 0; i < 10; i++ {
			if err := Get(dummyCtx, testKey, StringSink(&res)); err != nil {
				t.Fatal(err)
			}
		}
	}
	...
	for bytesFlooded < cacheSize+1024 {
		...
		Get(dummyCtx, key, StringSink(&res))
		bytesFlooded += int64(len(key) + len(res))
	}
	evicts := nevict - evict0
	if evicts <= 0 {
...
func TestPeers(t *testing.T) {
	...
	run := func(name string, n int, wantSummary string) {
		...
		for i := 0; i < n; i++ {
			...
			want := "got:" + key
			var got string
			err := Get(dummyCtx, key, StringSink(&got))
			if err != nil {
...
func TestTruncatingByteSliceTarget(t *testing.T) {
	var buf [100]byte
	s := buf[:]
	if err := Get(dummyCtx, "short", TruncatingByteSliceSink(&s)); err != nil {
		t.Fatal(err)
	}
	...
	s = buf[:6]
	if err := Get(dummyCtx, "truncated", TruncatingByteSliceSink(&s)); err != nil {
		t.Fatal(err)
	}
...
func TestNoDedup(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		go func() {
			var s string
			if err := Get(dummyCtx, testkey, StringSink(&s)); err != nil {
				resc <- "ERROR:" + err.Error()
				return
			}
			resc <- s
	...
	const wantItems = 1
	if items() != wantItems {
		t.Errorf("mainCache has %d items, want %d", items(), wantItems)
	}
	...
	if nbytes != wantBytes {
		t.Errorf("cache has %d bytes, want %d", nbytes, wantBytes)
	}
}

func TestGroupStatsAlignment(t *testing.T) {
	var g Group
	off := unsafe.Offsetof(Stats)
	if off%8 != 0 {
